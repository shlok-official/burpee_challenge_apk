...
public class Tile_wg {
   ...
   private final GameFragment_wg mGame;
   private Owner mOwner = Owner.NEITHER;
   private View mView;
   private Tile_wg mSubTileWgs[];

   public Tile_wg(GameFragment_wg game) {
      this.mGame = game;
   }

   public Tile_wg deepCopy() {
      Tile_wg tileWg = new Tile_wg(mGame);
      tileWg.setOwner(getOwner());
      if (getSubTiles() != null) {
         Tile_wg newTileWgs[] = new Tile_wg[9];
         Tile_wg oldTileWgs[] = getSubTiles();
         for (int child = 0; child < 9; child++) {
            newTileWgs[child] = oldTileWgs[child].deepCopy();
         }
         tileWg.setSubTiles(newTileWgs);
      }
      return tileWg;
   }
   ...
   public Tile_wg[] getSubTiles() {
      return mSubTileWgs;
   }

   public void setSubTiles(Tile_wg[] subTileWgs) {
      this.mSubTileWgs = subTileWgs;
   }
   ...
   private void countCaptures(int totalX[], int totalO[]) {
      ...
      for (int row = 0; row < 3; row++) {
         ...
         for (int col = 0; col < 3; col++) {
            Owner owner = mSubTileWgs[3 * row + col].getOwner();
      ...
      for (int diag = 0; diag < 3; diag++) {
         Owner owner = mSubTileWgs[3 * diag + diag].getOwner();
         if (owner == Owner.X || owner == Owner.BOTH) capturedX++;
         if (owner == Owner.O || owner == Owner.BOTH) capturedO++;
      }
      totalX[capturedX]++;
      ...
      capturedX = capturedO = 0;
      for (int diag = 0; diag < 3; diag++) {
         Owner owner = mSubTileWgs[3 * diag + (2 - diag)].getOwner();
         if (owner == Owner.X || owner == Owner.BOTH) capturedX++;
         if (owner == Owner.O || owner == Owner.BOTH) capturedO++;
      }
      totalX[capturedX]++;
